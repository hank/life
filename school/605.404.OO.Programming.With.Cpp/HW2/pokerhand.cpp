...
void PokerHand::checkStraight()
{
   ...
   if(Card::ACE == getHighCard().getCardRank() &&
      Card::TWO == getCard(0).getCardRank()
     )
   {
...
void PokerHand::checkPairs()
{
   ...
   while(i < NUMCARDS - 1)
   {
      ...
      const Card& originalCard = getCard(originalCardIdx);
   ...
   else
   {
      ...
      {   
         ...
         if(getFirstRank() == otherHand.getFirstRank())
         {
            ...
            {
               // All else failed.  Take the difference of the high card 
               // ranks.
      ...
      else
      {
         ...
         if(otherHand.getFirstRank() != getFirstRank())
         {
            // Simple case.  Return the difference of the first rank cases.
            result = (otherHand.getFirstRank() - getFirstRank());
         }
         else
         {
            // Special cases for iterative ranking
            if(getHandRank() == FLUSH ||
               getHandRank() == PAIR  ||
               getHandRank() == HIGH_CARD)
            {
               int i;
               int lastIndex = 0;
               // We have to iteratively compare the hands.
               for(i = NUMCARDS - 1; i >= lastIndex; i--)
               {
               ...
               // Check to see if everything was equal.
               // i will be less than lastIndex.
               if(i < lastIndex)
               {
                  // Completely equal.
                  result = 0;
               ...
               {
                  // Hands are different.  Return the difference of the cards.
                  result = otherHand.getCard(i).getCardRank() 
                                   - getCard(i).getCardRank();
               }
            }
            else
            {
               // Take the difference of the high card ranks.
               result = (otherHand.getHighCard().getCardRank() - 
                         getHighCard().getCardRank());
            }
         }
      }
   }
